...
## FBX axes, skeletons, and live player poses

Imported the implementation from -patch file here- on 2026-09-13 for Fabric 1.21.1 and 26.2. The supplied patch targets the full mod's earlier baseline; its source hunks apply to Lite's shared rig classes and modern client modules. This section replaces the patch's full-mod documentation and historical validation claims with the behavior and validation of this workspace.

- `com.aksulightning.fbxplayermodels.model.FbxCoordinateSpace` converts signed FBX `UpAxis` metadata to model-space +Y. Assimp and the binary fallback apply the same conversion before mesh scene transforms and through one skeleton wrapper root. Y-up files keep their orientation; X-up and Z-up files receive the appropriate signed rotation. Imported joint-local binds and animation keys keep their authored axes.
- Assimp preserves every hierarchy node, including unweighted joints and helper roots, and resolves skin influences by joint-node identity where available. The binary fallback preserves model parents, authored bind matrices, rotation/scaling pivots, and multiple meshes. Both paths retain all positive imported weights and leave unweighted vertices unchanged. Material recovery matches position/UV appearances onto Assimp geometry while preserving its skeleton and weights.
- Shared `LogicalRigBinding.resolveBoneIndex()` accepts explicit joint names, automatically fills unspecified parts, and leaves explicitly missing joints unresolved. Automatic binding ignores Assimp pivot helpers and prefers an authored head over a neck. Saving bindings regenerates procedural clips while retaining authored clips and explicit clip mappings.
- `ModelNormalizer` copies skinned geometry, normalizes skeleton roots once, preserves child-local binds, and recomputes inverse global binds. Absolute root animation keys receive the same normalization when sampled. Empty or degenerate bounds remain unchanged; the normalized model preserves bindings and the animation-enable setting.
- Each modern target's `CustomModelPose` supplies the same head and limb calculation to first- and third-person rendering. Head look uses wrapped, interpolated view/body yaw clamped to ±85 degrees and pitch clamped to ±90 degrees. It rotates in the authored bind basis around the fixed joint, inherits parent posture, applies during Idle/Walk/Sneak/Sit, and is suppressed during Sleep. Walking uses interpolated walk progress/amplitude; sitting, held items, attacks, and block breaking adjust the appropriate physical limbs. Live limb poses replace generated swing tracks even at zero rotation, and parent-first evaluation propagates posed transforms to descendants.
- Generated sneaking chest posture is additive around the authored bind pivot. Only a joint farther outside the mesh bounds than twice the mesh diagonal receives a geometry-derived posture pivot; bind matrices and weights remain intact. Skinning uses `posedGlobal * inverseBind` for positions and inverse-transpose matrices for normals, blends all valid influences, and normalizes the resulting normals. UVs, textures, and material colors are preserved. Disabling animations disables both clip playback and live bone poses.

The local-player rendering guards and single `SkinManager` cache remain in both modern targets. First-person rendering retains its camera, option, spectator, visibility, sleeping, and hidden-head restrictions. The 26.2 submission adapter also honors body/head pose overrides from the inventory preview. No server handling, remote-player models, networking, or Beta-specific implementation is added by this import; Beta continues to consume the shared rig-binding class.

Validation on 2026-09-13: the exact four Java/client compile tasks prescribed above passed using Java 25 via -path here-. Shared Java 17 sources and both modern client source sets compiled; both modern main compilation tasks have no Java sources. The initial sandboxed attempt could not write Gradle's existing wrapper cache lock; the approved retry completed successfully. No additional tests or build tasks were run. Interactive Minecraft rendering and imported FBX assets have not been exercised in this workspace.
