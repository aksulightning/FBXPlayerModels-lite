...
public class ModelNormalizer {
    ...
    public static SkinnedModel normalize(SkinnedModel model){
        ...
        for (SkinnedModel.Bone bone : model.bones) {
            // Normalization changes mesh/model space, not bone-local space.
            // Apply it once at skeleton roots; keep child-local joints intact.
            // Conjugating every local bind also transforms local zero, moving
            // rotated bone origins away from their authored attachment points.
